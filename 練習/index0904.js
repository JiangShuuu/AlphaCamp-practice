...
//   otherPlayers(winner, prize);
// }

// function otherPlayers(winner, prize) {
//   console.log(
...
function createPlayer(name, hp, mp) {
  return {
    ...
    cure: function (point) {
      ...
      if (this.hp > 0) {
        ...
        if (this.mp >= point * 2) {
          ...
          this.mp -= point * 2;
          console.log(
            `${this.name} HP is Recovered! (HP=${this.hp}, MP=${this.mp})`
          );
          return "==Change Sides==";
        } else if (this.mp < point * 2) {
          //mp值不夠補血
          return `${this.name}'s MP is not enough to recover HP! Change Sides.`;
        }
      } else {
        //hp值小於0
        return "GAME OVER!";
    ...
    attack: function (enemy) {
      // write your code
      const hurt = Math.floor(Math.random() * 100) + 1; //產生1~100隨機點數傷害
      enemy.hp = enemy.hp - hurt; //扣對手的hp
      console.log(
        `${this.name} hit ${enemy.name}. ${enemy.name} lose ${hurt} HP`
      );
      if (enemy.hp > 0) {
        //若對手hp > 0，回傳對手仍活著消息，否則回傳對手死亡消息
